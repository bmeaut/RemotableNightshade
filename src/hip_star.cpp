...
string Star1::getNameI18n(void) const {
  if (hip) {
    const string commonNameI18 = HipStarMgr::getCommonName(hip);
    if (!commonNameI18.empty()) return commonNameI18;
    if (HipStarMgr::getFlagSciNames()) {
      const string sciName = HipStarMgr::getSciName(hip);
      if (!sciName.empty()) return sciName;
      return "HP " + Utility::intToString(hip);
    }
  }
  return "";
}
