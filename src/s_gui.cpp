...
void Painter::drawSquareFill(const s_vec2i& pos, const s_vec2i& sz) const
{
	...
	glBegin(GL_QUADS );
	glTexCoord2s(0, 0);
	glVertex2i(pos[0]        , pos[1] + sz[1]);	// Bottom Left
	glTexCoord2s(1, 0);
	glVertex2i(pos[0] + sz[0], pos[1] + sz[1]);	// Bottom Right
	glTexCoord2s(1, 1);
	glVertex2i(pos[0] + sz[0], pos[1]        );	// Top Right
	glTexCoord2s(0, 1);
	glVertex2i(pos[0]        , pos[1]        );	// Top Left
	glEnd ();
}
...
string AutoCompleteString::getOptions(void)
{
...
string AutoCompleteString::getFirstOption(void)
{
	if (options.size() > 0)
		return options[0];
	else
...
// we need to set this when activated, and only blink the active one
...
bool EditBox::onKey(Uint16 k, S_GUI_VALUE s)
{
	...
	if (s==S_GUI_PRESSED) {
		...
		if(SDL_GetModState() & KMOD_CTRL) return false;  // Do not accept CTRL keystrokes
		...
		if (k == SDLK_TAB) {
			...
			// Macs don't have backspace keys, treat delete as backspace if we're at the end
			if(AppSettings::Instance()->OSX()) {
				if (cursorPos > 0) {
					cursorPos--;
					text = text.erase(cursorPos, 1);
				}
		...
		} else if (k == SDLK_ESCAPE) return 0;
		/*		else if ((k >= SDLK_0 && k <= SDLK_9) || (k >= SDLK_a && k <= SDLK_z)
		         || (k >= SDLK_A && k <= SDLK_Z) || (k >= 224 && k <= 255)
		         || k == SDLK_SPACE || k == SDLK_UNDERSCORE) {
		*/
		else if( k && k != SDLK_LSHIFT && !(SDL_GetModState() & KMOD_CTRL)
				   && k != SDLK_RSHIFT && !(SDL_GetModState() & KMOD_ALT) ) {
			text = lastText;
...
bool ScrollBar::onClic(int x, int y, S_GUI_VALUE bt, S_GUI_VALUE state)
{
	...
	if (visible && isIn(x,y) && state==S_GUI_PRESSED) {
		if (scrollBt.isIn(x-pos[0],y-pos[1])) {
			...
			if (n < scrollOffset) setValue(value - elementsForBar);
			else if (n > scrollOffset + scrollSize) setValue(value + elementsForBar);
...
/*  Comment below to allow dragging to continue even if leave bar area.
	However, never get mouse up event if outside scrollbar, so leaving alone for now
*/
	if (!isIn(x, y)) {
		dragging = false;
		return false;
...
void ScrollBar::setValue(int _value)
{
	value = _value;
	if(value < 0 ) value = 0;
	if(value > elements - elementsForBar) value = elements - elementsForBar;
...
void ListBox::setCurrent(const string& ws)
{
...
void ListBox::addItemList(const string& s)
{
	istringstream is(s);
	string elem;
...
// This is a modified copy of ListBox for a quick hack to make the help window
// easier to translate.  Not everything may work for other uses!

HashBox::HashBox(int _displayLines, float _ratio) :
		scrollBar(true),
		firstItemIndex(0),
		value(-1)
{
...
void HashBox::createLines(void)
{
	...
	for (i = 0; i < displayLines; i++) {
		...
		bt->adjustSize();
		bt->setVisible(visible);
		item2Bt.push_back(bt);
	}
}

void HashBox::draw(void)
{
	unsigned int i, j;
	...
	while (i < firstItemIndex + displayLines && i < items.size()) {
		...
		if (value != -1 && value == (int)i) {
			item2Bt[j]->setHideBorder(false);
			item2Bt[j]->setHideBorderMouseOver(false);
		} else {
			item2Bt[j]->setHideBorder(true);
			item2Bt[j]->setHideBorderMouseOver(true);
		}
		item2Bt[j++]->draw();
		i++;
	}
...
void HashBox::setCurrent(const string& ws)
{
	int i = firstItemIndex;
	vector<LabeledButton*>::iterator iter = itemBt.begin();
	while (iter != itemBt.end()) {
...
bool HashBox::onClic(int x, int y, S_GUI_VALUE button, S_GUI_VALUE state)
{
	if (!visible) return false;
	if (!isIn(x,y)) return false;
...
bool HashBox::onMove(int x, int y)
{
	if (!visible) return false;
	x = x - pos[0];
	y = y - pos[1];
...
void HashBox::setVisible(bool _visible)
{
	vector<LabeledButton*>::iterator iter = itemBt.begin();
	while (iter != itemBt.end()) {
		(*iter)->setVisible(_visible);
...
void HashBox::addItems(const vector<string> _items)
{
...
void HashBox::addItemList(const string& s)
{
	istringstream is(s);
	string elem1;
	string elem2;
	while (getline(is, elem1) && getline(is, elem2)) {
		addItem(elem1, elem2);
	}
}

void HashBox::addItem(const string& _text1, const string& _text2)
{
	if (!_text1.empty() || !_text2.empty()) {
		items.push_back(_text1);
		items2.push_back(_text2);

		adjustAfterItemsAdded();
	}
}

void HashBox::adjustAfterItemsAdded(void)
{
	createLines();
	setSize(getSizex(),LISTBOX_ITEM_HEIGHT * displayLines);
	scrollBar.setVisible(items.size() > displayLines);
	scrollBar.setOnChangeCallback(callback<void>(this, &HashBox::scrollChanged));
	scrollBar.setSize(SCROLL_SIZE,getSizey());
	scrollBar.setPos(getSizex()-SCROLL_SIZE,0);
	scrollBar.setTotalElements(items.size());
	scrollBar.setElementsForBar(displayLines);
...
void HashBox::scrollChanged(void)
{
	firstItemIndex = scrollBar.getValue();
	unsigned int i = firstItemIndex;
	unsigned int j = 0;
	...
	int w1 = int(w * splitRatio);
	int w2 = w - w1;

	while (i < firstItemIndex + displayLines && i < items.size()) {
		itemBt[j]->setPos(0,j*LISTBOX_ITEM_HEIGHT);
		itemBt[j]->setSize(w1,LISTBOX_ITEM_HEIGHT-1);
		itemBt[j]->setLabel(items[i]);

		item2Bt[j]->setPos(w1,j*LISTBOX_ITEM_HEIGHT);
		item2Bt[j]->setSize(w2,LISTBOX_ITEM_HEIGHT-1);
		item2Bt[j++]->setLabel(items2[i++]);
	}
}

void HashBox::clear(void)
{
	items.clear();
	adjustAfterItemsAdded();
}

string HashBox::getItem(int value)
{
	if (items.empty() || value < 0 || value >= (int)items.size())
		return string();
	else
...
void TextLabel::setLabel(const string& _label)
{
	...
	for(std::list<Component*>::const_iterator pos = childs.begin(); pos != childs.end(); ++pos)
		delete *pos;
	childs.clear();
...
StdWin::StdWin(const string& _title, const s_texture* _header_tex,
...
StdBtWin::StdBtWin(const string& _title, const s_texture* _header_tex,
                   s_font * _winfont, int headerSize) :
...
StdTransBtWin::StdTransBtWin(const string& _title, int _time_out,
...
StdDlgWin::StdDlgWin(const string& _title, const s_texture* _header_tex,
                     s_font * _winfont, int headerSize) :
...
{
	if (!visible) return;
	label->setLabel(Utility::doubleToString(value));
	Container::draw();
}
...
void FloatIncDec::draw()
{
	...
	if (format == FORMAT_DEFAULT) {
		...
		string l = Utility::printAngleDMS(value*M_PI/180.);
		string m = l.substr(1);
		...
		d(NULL), m(NULL), y(NULL), h(NULL), mn(NULL), s(NULL)
{
	...
	y = new IntIncDec(getFont(), tex_up, tex_down, -100000, 100000, 1930, 1);
	...
	Label* l1 = new Label(_("Year"));
	l1->setPos(5,5);
	y->setPos(50,6);
	y->setSize(50, 32);

	Label* l2 = new Label(_("Month"));
	l2->setPos(5,25);
	m->setPos(50,26);
	m->setSize(50, 32);

	Label* l3 = new Label(_("Day"));
	l3->setPos(5,45);
	d->setPos(50,44);
	d->setSize(50, 32);

	Label* l4 = new Label(_("Hour"));
	l4->setPos(130,5);
	h->setPos(190,6);
	h->setSize(50, 32);

	Label* l5 = new Label(_("Minutes"));
	l5->setPos(130,25);
	mn->setPos(190,26);
	mn->setSize(50, 32);

	Label* l6 = new Label(_("Seconds"));
	l6->setPos(130,45);
	s->setPos(190,46);
	s->setSize(50, 32);
...
{
	return NShadeDateTime::JulianDayFromDateTime(y->getValue(), m->getValue(), d->getValue(),
...
void MapPicture::setShowCities(bool show) {
	showCities = show;
}
