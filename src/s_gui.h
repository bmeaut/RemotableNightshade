...
public:
	...
	void setTexture(const s_texture* tex) {
		tex1 = tex;
	}
	void setFont(s_font* f) {
		font = f;
	}
	void setTextColor(const s_color& c) {
		textColor = c;
	}
	void setBaseColor(const s_color& c) {
		baseColor = c;
	}
	const s_color& getBaseColor(void) const {
		return baseColor;
	}
	const s_color& getTextColor(void) const {
		return textColor;
	}
	s_font* getFont(void) const {
		return font;
	}
	void setOpaque(bool _b) {
		opaque = _b;
	}
private:
...
public:
	...
	virtual void setSizex(int v) {
		size[0] = v;
	}
	virtual void setSizey(int v) {
		size[1] = v;
	}
	virtual void setPosx(int v) {
		pos[0] = v;
	}
	virtual void setPosy(int v) {
		pos[1] = v;
	}
	virtual const s_vec2i getPos() const {
		return pos;
	}
	virtual const s_vec2i getSize() const {
		return size;
	}
	virtual void setPos(const s_vec2i& _pos) {
		pos = _pos;
	}
	...
	virtual int getVisible(void) const {
		return visible;
	}
	virtual void setFocus(bool _focus) {
		focus = _focus;
	};
	virtual bool getFocus(void) const {
		return focus;
	}
	...
	virtual void setTexture(const s_texture* tex) {
		painter.setTexture(tex);
	}
	virtual void setFont(s_font* f) {
		painter.setFont(f);
	}
	s_font* getFont(void) const {
	...
	virtual void setBaseColor(const s_color& c) {
		painter.setBaseColor(c);
	}
	virtual void setPainter(const Painter& p) {
		painter = p;
	}
	virtual bool isIn(int x, int y);
	static void setDefaultPainter(const Painter& p) {
		defaultPainter=p;
	}
	static void initScissor(int winW, int winH);
	static void deleteScissor(void);
	static void enableScissor(void) {
		scissor->activate();
	}
	static void disableScissor(void) {
		scissor->desactivate();
	}
	bool inFront(void) {
		return moveToFront;
	}
	bool getNeedNewEdit(void) {
		return needNewTopEdit;
	}
	void setNeedNewEdit(bool _b) {
		needNewTopEdit = _b;
	}
	void setInFront(bool b) {
		moveToFront = b;    // signals this component to move to front
	}
	...
	virtual void setGUIColorSchemeMember(bool _b) {
...
public:
	CallbackComponent();
	virtual void setOnMouseInOutCallback(const callback<void>& c) {
		onMouseInOutCallback = c;
	}
	virtual void setOnPressCallback(const callback<void>& c) {
		onPressCallback = c;
	}
	virtual bool getIsMouseOver(void) {
...
public:
	...
	virtual void draw();
	virtual bool onClic(int, int, S_GUI_VALUE, S_GUI_VALUE);
	void setHideBorder(bool _b) {
		hideBorder = _b;
	}
	void setHideBorderMouseOver(bool _b) {
		hideBorderMouseOver = _b;
	}
	void setHideTexture(bool _b) {
		hideTexture = _b;
	}
protected:
	bool hideBorder;
...
public:
	...
	virtual void setJustification(Justification _j) {
		justification = _j;
	}
	virtual void adjustSize(void);
...
public:
	...
	virtual ~EditBox();
	virtual void setOnReturnKeyCallback(const callback<void>& c) {
		onReturnKeyCallback = c;
	}
	virtual void setOnKeyCallback(const callback<void>& c) {
		onKeyCallback = c;
	}
	virtual void setOnAutoCompleteCallback(const callback<void>& c) {
	...
	void setAutoCompleteOptions(vector<string> _autocomplete) {
		autoComplete.setOptions(_autocomplete);
	};
	string getAutoCompleteOptions(void) {
	...
	Uint16 getLastKey(void) {
		return lastKey;
	}
	void setAutoFocus(bool _b) {
		autoFocus = _b;
	}
	bool getAutoFocus(void) {
		return autoFocus;
	}
...
public:
	...
	virtual void setOnChangeCallback(const callback<void>& c) {
		onChangeCallback = c;
	}
	virtual void draw(void);
	virtual bool onClic(int, int, S_GUI_VALUE, S_GUI_VALUE);
...
public:
	...
	void addItemList(const string& _text);
	void addItem(const string& _text);
	void clear(void);
	int getValue(void) {
		return value;
	}
	void setCurrent(const string& ws);
private:
	callback<void> onChangeCallback;
	ScrollBar scrollBar;
...
class HashBox : public Component
{
public:
	HashBox(int _displayLines = 5, float _rtio=0.5);
	~HashBox();
	virtual void setOnChangeCallback(const callback<void>& c) {
		onChangeCallback = c;
	}
	...
	void addItems(const vector<string> _items);
	void addItemList(const string& _text);
	void addItem(const string& _text1, const string& _text2);
	void clear(void);
	int getValue(void) {
		return value;
	}
...
public:
	...
	virtual void setOnChangeTimeCallback(callback<void> c) {
...
{
public:
	StdWin(const string& _title = "", const s_texture* _header_tex = NULL,
...
{
public:
	StdDlgWin(const string& _title = NULL,
	          const s_texture* _header_tex = NULL,
	          s_font * _winfont = NULL, int headerSize = 18);
	~StdDlgWin();
	virtual void setDialogCallback(const callback<void>& c) {
	...
	virtual void setOnCompleteCallback(const callback<void>& c) {
		onCompleteCallback = c;
	}
	string getLastID(void) {
		return lastID;
	}
...
{
public:
	StdBtWin(const string& _title = NULL,
	         const s_texture* _header_tex = NULL,
	         s_font * _winfont = NULL, int headerSize = 18);
	virtual void draw();
	virtual void setOnHideBtCallback(const callback<void>& c) {
		onHideBtCallback = c;
	}
protected:
	void onHideBt(void);
...
{
public:
	StdTransBtWin(const string& _title = "", int _time_out =0,
	              const s_texture* _header_tex = NULL,
	              s_font * _winfont = NULL, int headerSize = 18);
...
public:
	...
	~Picture();
	virtual void draw(void);
	void setShowEdge(bool v) {
		showedges = v;
	}
	void setImgColor(const s_color &c) {
		imgcolor = c;
	}
private:
...
public:
	...
	string getNameI18(void) {
		return _(name);
	}
	string getState(void) {
		return state;
	}
	string getStateI18(void) {
		return _(state.c_str());
	}
	string getCountry(void) {
		return country;
	}
	string getCountryI18(void) {
...
public:
	MapPicture(const s_texture *_imageTex,
	           ...
	           int xpos = 0, int ypos = 0, int xsize = 32, int ysize = 32);
	~MapPicture();
	virtual void setOnNearestCityCallback(const callback<void>& c) {
		onNearestCityCallback = c;
	}
	virtual void draw(void);
	virtual bool onClic(int, int, S_GUI_VALUE, S_GUI_VALUE);
	virtual bool onMove(int, int);
	...
	void setPointerLongitude(double _longitude) {
		pointerPos[0] = getxFromLongitude(_longitude);
	};
	void setPointerLatitude(double _latitude) {
		pointerPos[1] = getyFromLatitude(_latitude);
	};
	...
	void addCity(const string& _name, const string& _state, const string& _country,
		...
		cities.addCity(_name, _state, _country, _longitude, _latitude, _zone, _showatzoom, _altitude);
	}
