/////////////////////////////////////////////////////////////////////////////
// Copyright (c) 2010-2013 Piotr Likus
// Name:        mongcpp.cpp
// Project:     mongoose
...
void MongooseServer::checkMethodMap()
{
    if (m_methodMap.get() == NULL)
    {
        ...
        defineMethod("GET", rmcGet);
        defineMethod("POST", rmcPost);
        defineMethod("HEAD", rmcHead);
        defineMethod("PUT", rmcPut);
        defineMethod("DELETE", rmcDelete);
        defineMethod("TRACE", rmcTrace);
        defineMethod("OPTIONS", rmcOptions);
    }
}

void MongooseServer::defineMethod(const std::string &methodText, MongooseRequestMethodCode methodCode)
{
    m_methodMap->insert(std::make_pair(methodText, methodCode));
}
