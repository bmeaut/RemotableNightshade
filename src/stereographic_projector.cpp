        ...
        const Mat4d &mat) const
{
	...
	win[0] = viewport_center[0] + flip_horz * x * f;
	win[1] = viewport_center[1] + flip_vert * y * f;
...
{
	x = flip_horz * (x - viewport_center[0]) / (view_scaling_factor*2);
	y = flip_vert * (y - viewport_center[1]) / (view_scaling_factor*2);
