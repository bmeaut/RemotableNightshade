...
void SphericMirrorProjector::setViewport(int x, int y, int w, int h)
{
	...
	viewport_center.set(vec_viewport[0]+vec_viewport[2]/2,
        ...
        const Mat4d &mat) const
{
	...
	x[0] = viewport_center[0] - flip_horz * x[0] * (5*view_scaling_factor);
	x[1] = viewport_center[1] + flip_vert * x[1] * (5*view_scaling_factor);
...
{
	x = - flip_horz * (x - viewport_center[0]) / (5*view_scaling_factor);
	y = flip_vert * (y - viewport_center[1]) / (5*view_scaling_factor);
