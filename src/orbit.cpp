...
Vec3d CometOrbit::positionAtTime(double JD) const
{
	...
	// Only supports parabolic (e=1) orbits
	if(e != 1) {
		cerr << "ERROR: CometOrbit with non-parabolic orbit!\n";
		return( Vec3d(0,0,0) );
	}
										   ...
										   bool useParentPrecession)
{
    ...
    // Compute the period
    //double T = 2 * M_PI * sqrt((a*a*a) / GM);
    //T = T / 86400.0; // Convert from seconds to days
	//	double a2 = pow(GM * pow(86400*T/2.0/M_PI, 2), 1.0/3.0);


	//cout << "a: " << a << " " << (a * (1 - e))/AU << endl;
	//cout << "e: " << e << "\ni: " << i << "\nOm: " << Om << "\nom: " << om << "\nM: " << M << "\nT: " << T << "\nt: " << t << endl;
